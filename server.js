/* =========================================================
   HARVEY TAXI — CODE BLUE CLEAN MERGED SERVER
...
const ENABLE_AI_SUPPORT = toBool(process.env.ENABLE_AI_SUPPORT, true);
...
const DISPATCH_TIMEOUT_SECONDS = Number(
  process.env.DISPATCH_TIMEOUT_SECONDS || 30
);
const MAX_DISPATCH_ATTEMPTS = Number(
...
const DRIVER_PAYOUT_PERCENT = Number(
  process.env.DRIVER_PAYOUT_PERCENT || 0.75
);
...
let openai = null;
if (ENABLE_AI && ENABLE_AI_SUPPORT && OpenAI && OPENAI_API_KEY) {
  try {
    openai = new OpenAI({
...
function normalizeRequestedMode(value = "") {
  ...
  if (["autonomous", "av", "pilot", "self-driving"].includes(mode)) {
    return "autonomous";
  }
  return "driver";
}
...
app.get("/api/health", async (req, res) => {
  ...
  res.json({
   ...
   PART 2: AI SUPPORT + RIDER + DRIVER + PAYMENT + FARE HELPERS
========================================================= */
...
function normalizePage(page = "") {
  ...
  if (
    ["rider", "driver", "request", "support", "admin", "general"].includes(
      value
    )
  ) {
    return value;
  ...
  if (["rider-signup", "rider-dashboard"].includes(value)) return "rider";
  if (["driver-signup", "driver-dashboard"].includes(value)) return "driver";
  if (["request-ride", "ride"].includes(value)) return "request";
  if (["help", "faq"].includes(value)) return "support";
...
async function generateOpenAISupportReply({
  ...
  if (!ENABLE_AI_SUPPORT || !openai) return null;
...
app.post("/api/ai/support", async (req, res) => {
  try {
    ...
    return res.json({
      ...
      reply,
      meta: {
        ai_enabled: !!(ENABLE_AI_SUPPORT && openai),
        page,
        source
      },
      timestamp: now()
...
function buildFareEstimate({
  ...
  const driver_payout_estimate = toMoney(estimated_fare * DRIVER_PAYOUT_PERCENT);
...
app.post("/api/fare-estimate", async (req, res) => {
  try {
   ...
   PART 3: RIDER ROUTES + DRIVER ROUTES + DISPATCH / TRIP HELPERS
========================================================= */
...
async function updateRideStatus(rideId = "", status = "") {
  return patchRide(rideId, {
    status: cleanEnv(status)
  });
}
...
async function getRideForDriver(driverId = "") {
  ...
  const { data, error } = await supabase
    ...
    .in("status", ["dispatched", "driver_en_route", "arrived", "in_progress"])
...
async function createDispatchOfferForRide(rideId = "") {
  ...
  if (existingMission) {
    ...
    if (
      latestDispatch &&
      !dispatchIsExpired(latestDispatch) &&
      cleanEnv(latestDispatch.status) === "offered"
    ) {
      return {
        ok: true,
...
async function sweepExpiredDispatches() {
  ...
  try {
   ...
   PART 4: REQUEST RIDE + MISSION FLOW + RIDE LIFECYCLE
   + TIPS + EARNINGS + ADMIN + FINAL BOOT
========================================================= */
...
function adminAuthorized(req) {
  ...
  return !!ADMIN_EMAIL && !!ADMIN_PASSWORD && email === ADMIN_EMAIL && password === ADMIN_PASSWORD;
}
...
app.listen(PORT, () => {
  ...
  console.log(
    `✅ Admin password loaded: ${ADMIN_PASSWORD ? "yes" : "no"}`
  );
  console.log("==================================================");
});
