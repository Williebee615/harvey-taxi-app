/* =========================================================
   HARVEY TAXI — PRODUCTION CODE BLUE SERVER.JS
   PART 1 OF 9
   FOUNDATION + ENV + CLIENTS + SECURITY + HELPERS
...
try {
  ...
  console.warn("⚠️ Missing @sendgrid/mail. Install with: npm install @sendgrid/mail");
}
...
try {
  ...
  console.warn("⚠️ Missing twilio. Install with: npm install twilio");
}
...
try {
  ...
  console.warn("⚠️ Missing stripe. Install with: npm install stripe");
}
...
try {
  ...
  console.warn("⚠️ Missing openai. Install with: npm install openai");
}
...
if (sgMail && SENDGRID_API_KEY && ENABLE_REAL_EMAIL) {
  ...
  console.warn("⚠️ SendGrid not active.");
}
...
if (
  ...
  console.warn("⚠️ Twilio not active.");
}
...
if (Stripe && STRIPE_SECRET_KEY) {
  ...
  console.warn("⚠️ Stripe not active.");
}
...
if (CHECKR_API_KEY && ENABLE_CHECKR) {
  console.log("✅ Checkr configured.");
} else {
  console.warn("⚠️ Checkr not active.");
}
...
if (OpenAI && OPENAI_API_KEY && ENABLE_AI_SUPPORT) {
  ...
  console.warn("⚠️ OpenAI not active.");
}
...
/* =========================================================
   SECURITY MIDDLEWARE
   IMPORTANT:
   Stripe webhook raw route must be declared before express.json
   in the Stripe section later.
========================================================= */
...
app.use((req, res, next) => {
  ...
  res.setHeader("x-harvey-request-id", req.requestId);
  next();
});
...
function requireAdmin(req, res, next) {
  ...
  const emailOk = email === normalizeEmail(ADMIN_EMAIL);
...
/* =========================================================
   PART 1 END
   NEXT: PART 2 — RIDERS + DRIVER ONBOARDING
========================================================= *//* =========================================================
   HARVEY TAXI — PRODUCTION CODE BLUE SERVER.JS
   PART 2 OF 9
   RIDERS + DRIVER ONBOARDING + SENDGRID EMAIL VERIFY
   SMS VERIFY + STATUS ROUTES
========================================================= */
...
async function sendEmail({ to, subject, html, text }) {
  ...
  if (!sgMail || !SENDGRID_API_KEY || !ENABLE_REAL_EMAIL) {
    console.warn("📧 Email not sent. SendGrid inactive:", { to, subject });
    return { sent: false, inactive: true };
  }
  ...
  return `
    <div style="font-family:Arial,sans-serif;background:#f6f8fb;padding:24px;">
      <div style="max-width:640px;margin:auto;background:#ffffff;border:1px solid #e5e7eb;border-radius:18px;padding:30px;">
        <h2 style="margin:0 0 12px;color:#0f172a;">Verify your Harvey Taxi driver email</h2>
        <p style="font-size:16px;color:#334155;">
          Hi ${driverName || "Driver"},
        </p>
        <p style="font-size:16px;color:#334155;">
          Thank you for starting your Harvey Taxi driver onboarding.
          Please verify your email address so we can continue your approval process.
        </p>
...
async function sendDriverEmailVerification(driver) {
  ...
  return {
    ...result,
    expiresAt,
    verifyUrl: !ENABLE_REAL_EMAIL ? verifyUrl : undefined,
  };
}
...
async function sendSms({ to, body }) {
  ...
  if (!twilioClient || !ENABLE_REAL_SMS) {
    console.warn("📱 SMS not sent. Twilio inactive:", { to, body });
    return { sent: false, inactive: true };
  }
...
app.post("/api/riders/signup", async (req, res) => {
  try {
    ...
    return ok(
      ...
      {
        message:
          "Rider signup received. Rider verification approval is required before requesting rides.",
        rider: publicRider(rider),
        canRequestRide: !ENABLE_RIDER_APPROVAL_GATE,
      },
      201
    );
  } catch (error) {
...
app.post("/api/drivers/signup", async (req, res) => {
  try {
    ...
    const driverType = normalizeText(req.body.driver_type || req.body.driverType || "human");
    const licenseNumber = normalizeText(req.body.license_number || req.body.licenseNumber);
    const vehicleMake = normalizeText(req.body.vehicle_make || req.body.vehicleMake);
    const vehicleModel = normalizeText(req.body.vehicle_model || req.body.vehicleModel);
    const vehicleYear = normalizeText(req.body.vehicle_year || req.body.vehicleYear);
    const zipcode = normalizeText(req.body.zipcode || req.body.postal_code || req.body.postalCode);
    ...
    if (existing) {
      if (!existing.email_verified) {
        ...
        return ok(res, {
          message:
            "Driver already exists. A new email verification link has been sent.",
          driver: publicDriver(existing),
          emailVerificationSent: !!resend.sent,
          devVerifyUrl: resend.verifyUrl,
        });
      }
    ...
    return ok(
      ...
      {
        message:
          "Driver signup received. Verify your email to continue driver onboarding.",
...
app.get("/api/drivers/verify-email", async (req, res) => {
  try {
    ...
    const savedHash = driver.email_verification_token_hash;
    const expiresAt = driver.email_verification_expires_at;

    if (!savedHash || !safeCompare(tokenHash, savedHash)) {
    ...
    return res.send(`
      <html>
        <body style="font-family:Arial;background:#f8fafc;color:#0f172a;padding:32px;">
          <div style="max-width:680px;margin:auto;background:white;border:1px solid #e2e8f0;border-radius:16px;padding:30px;">
            ...
            <p>The next step is phone verification and background-check review.</p>
          </div>
        </body>
      </html>
    `);
...
app.post("/api/drivers/send-sms-code", async (req, res) => {
  try {
    ...
    return ok(res, {
      ...
      expiresAt,
      devCode: !ENABLE_REAL_SMS ? code : undefined,
    });
  } catch (error) {
...
app.post("/api/drivers/verify-sms-code", async (req, res) => {
  try {
    ...
    const savedHash = driver.phone_verification_code_hash;
    const expiresAt = driver.phone_verification_expires_at;

    if (!savedHash || !safeCompare(sha256(code), savedHash)) {
      return fail(res, 401, "Invalid SMS verification code.");
    }

    if (expiresAt && new Date(expiresAt).getTime() < Date.now()) {
      return fail(res, 401, "SMS verification code expired.");
    }
...
app.get("/api/drivers/status", async (req, res) => {
  try {
    const driverId = safeTrim(req.query.driverId || req.query.driver_id || req.query.id);
    const email = normalizeEmail(req.query.email);
    ...
    const emailOk = !!driver.email_verified;
    const phoneOk = ENABLE_REAL_SMS ? !!driver.phone_verified : true;
    const checkrOk = [
      "clear",
      "report_clear",
      "complete",
    ...
    return ok(res, {
      ...
      onboarding: {
        ...
        phoneRequired: ENABLE_REAL_SMS,
        checkrStatus: driver.checkr_status || "not_started",
        checkrPassed: checkrOk,
        adminApproved: approved,
      },
      canAcceptMissions: emailOk && phoneOk && checkrOk && approved,
    });
  } catch (error) {
...
/* =========================================================
   PART 2 END
   NEXT: PART 3 — CHECKR CANDIDATE + INVITATION + WEBHOOK
========================================================= *//* =========================================================
   HARVEY TAXI — PRODUCTION CODE BLUE SERVER.JS
   PART 3 OF 9
   CHECKR CANDIDATE + INVITATION + WEBHOOK
   DRIVER BACKGROUND CHECK FLOW
========================================================= */
...
function normalizeCheckrStatus(eventType, object = {}) {
  const rawStatus = normalizeText(object.status || "");
  const result = normalizeText(object.result || "");

  if (eventType.includes("invitation.completed")) {
    return "invitation_completed";
  }

  if (eventType.includes("invitation.expired")) {
  ...
  if (eventType.includes("report.suspended")) {
    return "report_suspended";
  }

  if (eventType.includes("report.canceled")) {
...
function deriveDriverApprovalFromCheckr(checkrStatus) {
  if (["report_clear", "clear", "completed", "complete"].includes(checkrStatus)) {
    return {
...
async function createCheckrCandidateForDriver(driver) {
  ...
  const candidate = await checkrRequest("/candidates", {
    ...
    body: {
      ...
      email: driver.email,
      phone: driver.phone,
      zipcode: driver.zipcode || driver.postal_code || "37201",
  ...
  await auditLog("checkr_candidate_created", {
    driver_id: driver.id,
    checkr_candidate_id: candidate.id,
  });
...
async function createCheckrInvitationForDriver(driver, candidateId) {
  ...
  await auditLog("checkr_invitation_created", {
    driver_id: driver.id,
...
async function startDriverCheckrFlow(driver) {
  if (!driver.email_verified) {
    throw new Error("Driver email must be verified before starting Checkr.");
  }

  if (ENABLE_REAL_SMS && !driver.phone_verified) {
    throw new Error("Driver phone must be verified before starting Checkr.");
  }
  ...
  if (!candidateId) {
    candidate = await createCheckrCandidateForDriver(driver);
    candidateId = candidate.id;
  }

  const invitation = await createCheckrInvitationForDriver(driver, candidateId);
...
app.post("/api/drivers/start-checkr", async (req, res) => {
  try {
    ...
    return serverError(res, error, "Could not start Checkr background check.");
  }
});

/* =========================================================
   ADMIN START CHECKR ROUTE
========================================================= */

app.post("/api/admin/drivers/start-checkr", requireAdmin, async (req, res) => {
  try {
    const driverId = safeTrim(req.body.driverId || req.body.driver_id);
    ...
    await auditLog("admin_started_checkr", {
      admin: req.admin.email,
      driver_id: driver.id,
      checkr_candidate_id: result.candidate?.id,
      checkr_invitation_id: result.invitation?.id,
    });

    return ok(res, {
      message: "Admin started Checkr background-check flow.",
      driver: publicDriver({
        ...driver,
        checkr_status: "invitation_sent",
        status: "background_check_invited",
    ...
    return serverError(res, error, "Admin could not start Checkr.");
  }
});
...
app.post("/api/webhooks/checkr", async (req, res) => {
  try {
    if (!verifyCheckrWebhook(req)) {
      return fail(res, 401, "Invalid Checkr webhook signature.");
    }
    ...
    const approvalUpdate = deriveDriverApprovalFromCheckr(checkrStatus);
...
/* =========================================================
   PART 3 END
   NEXT: PART 4 — FARE ESTIMATE + STRIPE PAYMENT AUTHORIZATION
========================================================= *//* =========================================================
   HARVEY TAXI — PRODUCTION CODE BLUE SERVER.JS
   PART 4 OF 9
   FARE ESTIMATE + GEO HELPERS + STRIPE PAYMENT AUTHORIZATION
   PAYMENT GATE BEFORE DISPATCH
========================================================= */
...
/* =========================================================
   ...
   Creates Stripe PaymentIntent in manual capture mode.
========================================================= */
...
app.post("/api/payments/authorize", async (req, res) => {
  try {
    ...
    if (ENABLE_RIDER_APPROVAL_GATE && !riderApproved) {
      return fail(res, 403, "Rider must be approved before payment authorization.");
    }

    if (!ENABLE_PAYMENT_GATE) {
      const devPayment = await dbInsert("payments", {
        rider_id: rider.id,
        amount: 0,
        currency: "usd",
        status: "authorized",
    ...
    return serverError(res, error, "Could not authorize payment.");
  }
});
...
/* =========================================================
   PART 4 END
   NEXT: PART 5 — RIDE REQUEST + PAYMENT GATE + DISPATCH ENGINE + MISSIONS
========================================================= *//* =========================================================
   HARVEY TAXI — PRODUCTION CODE BLUE SERVER.JS
   PART 5 OF 9
   RIDE REQUEST + PAYMENT GATE + DISPATCH ENGINE + MISSIONS
========================================================= */

/* =========================================================
   RIDE PAYMENT AUTHORIZATION CHECK
========================================================= */
...
async function assertRidePaymentAuthorized({ riderId, paymentId }) {
  ...
  if (!authorizedStatuses.includes(finalStatus)) {
    throw new Error(
      `Payment is not authorized. Current status: ${finalStatus}`
    );
  }
...
function driverCanReceiveMission(driver = {}) {
  ...
  return emailOk && phoneOk && approved && checkrOk && available;
}
...
function scoreDriverForRide(driver, ride) {
  ...
  if (distanceMiles > DRIVER_SEARCH_RADIUS_MILES) {
    return null;
  }
...
async function dispatchRide(rideId) {
  ...
  if (
    ["completed", "canceled", "driver_assigned", "driver_en_route", "driver_arrived", "in_progress"].includes(
      ride.status
    )
  ) {
    return {
  ...
  for (const dispatch of priorDispatches || []) {
    alreadyOfferedDriverIds.add(dispatch.driver_id);
  }
...
app.post("/api/request-ride", async (req, res) => {
  try {
    ...
    if (ENABLE_RIDER_APPROVAL_GATE && !riderApproved) {
      return fail(res, 403, "Rider verification approval is required before requesting a ride.");
    }
...
app.post("/api/admin/rides/redispatch", requireAdmin, async (req, res) => {
  try {
    ...
    if (!rideId) {
      return fail(res, 400, "Ride ID is required.");
    }
...
/* =========================================================
   PART 5 END
   NEXT: PART 6 — DRIVER DASHBOARD + TRIP LIFECYCLE
========================================================= *//* =========================================================
   HARVEY TAXI — PRODUCTION CODE BLUE SERVER.JS
   PART 6 OF 9
   DRIVER DASHBOARD + LIVE LOCATION + TRIP LIFECYCLE
========================================================= */
...
app.post("/api/driver/availability", async (req, res) => {
  try {
    ...
    const canReceive = driverCanReceiveMission({
      ...driver,
      available: true,
    });

    if (available && !canReceive) {
    ...
    await auditLog("driver_availability_changed", {
      driver_id: driver.id,
      available,
    });
...
app.post("/api/driver/location", async (req, res) => {
  try {
    ...
    const heading = toNumber(req.body.heading, null);
    const speed = toNumber(req.body.speed, null);
...
app.get("/api/driver/dashboard", async (req, res) => {
  try {
    ...
    return ok(res, {
      driver: publicDriver(driver),
      canReceiveMissions: driverCanReceiveMission(driver),
      activeMission: activeMissions?.[0] || null,
      activeRide: activeRides?.[0] || null,
      recentEarnings: earnings || [],
      recentEarningsTotal: roundMoney(earningsTotal),
    });
  } catch (error) {
    return serverError(res, error, "Could not load driver dashboard.");
  }
});
...
app.get("/api/rides/status", async (req, res) => {
  try {
    ...
    if (ride.driver_id) {
      driver = await dbFindById("drivers", ride.driver_id);
    }
...
app.post("/api/driver/trip/complete", async (req, res) => {
  try {
    ...
    const existingEarning = await dbFindOne("driver_earnings", "ride_id", ride.id);

    let earning = existingEarning;
...
/* =========================================================
   COMPATIBILITY ROUTES FOR EXISTING FRONTEND BUTTONS
========================================================= */
...
/* =========================================================
   PART 6 END
   NEXT: PART 7 — TIPPING + EARNINGS + PAYOUT LEDGER
========================================================= *//* =========================================================
   HARVEY TAXI — PRODUCTION CODE BLUE SERVER.JS
   PART 7 OF 9
   TIPPING + DRIVER EARNINGS + PAYOUT LEDGER
========================================================= */
...
app.post("/api/rides/tip", async (req, res) => {
  try {
    ...
    if (!["driver_assigned", "driver_en_route", "driver_arrived", "in_progress", "completed"].includes(ride.status)) {
...
/* =========================================================
   COMPATIBILITY TIP ROUTE
========================================================= */

app.post("/api/rider/tip", async (req, res) => {
...
app.get("/api/rider/tips", async (req, res) => {
  try {
    const riderId = safeTrim(req.query.rider_id || req.query.riderId);
    ...
    return serverError(res, error, "Could not load rider tips.");
  }
});

/* =========================================================
   MARK EARNINGS AVAILABLE
   This can be run manually by admin or later by a scheduled job.
========================================================= */
...
app.post("/api/admin/earnings/mark-available", requireAdmin, async (req, res) => {
  try {
    ...
    if (!earningId) {
      return fail(res, 400, "Earning ID is required.");
    }

    const earning = await dbFindById("driver_earnings", earningId);

    if (!earning) {
      return fail(res, 404, "Earning not found.");
    }
...
app.post("/api/admin/payouts/create", requireAdmin, async (req, res) => {
  try {
    ...
    if (!earningIds.length) {
      return fail(res, 400, "At least one earning ID is required.");
    }
    ...
    if (!earnings?.length) {
      return fail(res, 404, "No matching earnings found.");
    }

    const invalid = earnings.filter((earning) => earning.status !== "available");

    if (invalid.length) {
      return fail(res, 409, "Only available earnings can be included in a payout.", {
        invalidEarningIds: invalid.map((e) => e.id),
      });
    }

    const total = roundMoney(
      earnings.reduce((sum, earning) => sum + Number(earning.amount || 0), 0)
    );
    ...
    return ok(res, {
      message: "Driver payout record created.",
      payout,
    });
  } catch (error) {
...
app.post("/api/admin/payouts/mark-paid", requireAdmin, async (req, res) => {
  try {
    ...
    const reference = normalizeText(req.body.reference || req.body.transaction_reference);

    if (!payoutId) {
      return fail(res, 400, "Payout ID is required.");
    }
    ...
    const updatedPayout = await dbUpdateById("driver_payouts", payout.id, {
      status: "paid",
      paid_at: paidAt,
...
/* =========================================================
   PART 7 END
   NEXT: PART 8 — ADMIN DASHBOARD + ANALYTICS + AUDIT LOGS
========================================================= *//* =========================================================
   HARVEY TAXI — PRODUCTION CODE BLUE SERVER.JS
   PART 8 OF 9
   ADMIN DASHBOARD + ANALYTICS + AUDIT LOGS + OPS CONTROLS
========================================================= */
...
app.get("/api/admin/dashboard", requireAdmin, async (req, res) => {
  try {
    ...
    const capturedRevenue = payments
      .filter((p) => ["succeeded", "captured"].includes(p.status))
      .reduce((sum, p) => sum + Number(p.amount || 0), 0);

    const pendingDriverEarnings = earnings
      .filter((e) => ["pending", "available", "payout_pending"].includes(e.status))
      .reduce((sum, e) => sum + Number(e.amount || 0), 0);

    const paidDriverPayouts = payouts
    ...
    return ok(res, {
      dashboard: {
        ...
        drivers: {
          ...
          manualReview: drivers.filter((d) => d.approval_status === "manual_review").length,
          approved: drivers.filter((d) => d.approval_status === "approved").length,
          online: drivers.filter((d) => d.available === true).length,
        },
        rides: {
          total: rides.length,
          ...
          noDriverAvailable: rides.filter((r) => r.status === "no_driver_available").length,
        },
        money: {
          capturedRevenue: roundMoney(capturedRevenue),
          pendingDriverEarnings: roundMoney(pendingDriverEarnings),
          paidDriverPayouts: roundMoney(paidDriverPayouts),
        },
      },
    });
  } catch (error) {
...
app.get("/api/admin/rides/detail", requireAdmin, async (req, res) => {
  try {
    const rideId = safeTrim(req.query.ride_id || req.query.rideId);
    ...
    const [rider, driver, paymentResult, missionResult, dispatchResult, eventResult] =
      await Promise.all([
        ride.rider_id ? dbFindById("riders", ride.rider_id) : null,
        ride.driver_id ? dbFindById("drivers", ride.driver_id) : null,
        supabase.from("payments").select("*").eq("ride_id", ride.id),
        supabase.from("missions").select("*").eq("ride_id", ride.id),
        supabase.from("dispatches").select("*").eq("ride_id", ride.id),
        supabase
          .from("trip_events")
          .select("*")
          .eq("ride_id", ride.id)
    ...
    return serverError(res, error, "Could not load ride detail.");
  }
});
...
app.get("/api/admin/payments", requireAdmin, async (req, res) => {
  try {
    ...
    return serverError(res, error, "Could not load payments.");
  }
});
...
app.get("/api/admin/audit-logs", requireAdmin, async (req, res) => {
  try {
    const eventType = normalizeText(req.query.event_type || req.query.eventType);
    const limit = Math.min(toNumber(req.query.limit, 100), 500);
    ...
    return serverError(res, error, "Could not load audit logs.");
  }
});
...
app.post("/api/admin/rides/cancel", requireAdmin, async (req, res) => {
  try {
    ...
    if (ride.payment_id) {
      try {
        payment = await cancelRidePayment(ride.payment_id, "requested_by_customer");
      } catch (paymentError) {
    ...
    await rideEvent(ride.id, "admin_ride_canceled", {
      admin: req.admin.email,
      reason,
    });
...
app.post("/api/admin/rides/assign-driver", requireAdmin, async (req, res) => {
  try {
    const rideId = safeTrim(req.body.ride_id || req.body.rideId);
    const driverId = safeTrim(req.body.driver_id || req.body.driverId);

    if (!rideId || !driverId) {
      return fail(res, 400, "Ride ID and driver ID are required.");
    }
    ...
    if (!driverCanReceiveMission(driver)) {
      return fail(res, 403, "Driver is not eligible for assignment.", {
        driver: publicDriver(driver),
      });
    }

    if (["completed", "canceled", "in_progress"].includes(ride.status)) {
      return fail(res, 409, `Ride cannot be assigned from status ${ride.status}.`);
    }
    ...
    await auditLog("admin_assigned_driver", {
      admin: req.admin.email,
      ride_id: ride.id,
      driver_id: driver.id,
      mission_id: mission.id,
    ...
    await rideEvent(ride.id, "admin_assigned_driver", {
      admin: req.admin.email,
      driver_id: driver.id,
      mission_id: mission.id,
    });
    ...
    return serverError(res, error, "Could not assign driver.");
  }
});
...
app.get("/api/admin/analytics", requireAdmin, async (req, res) => {
  try {
    ...
    const capturedPayments = (payments || []).filter((p) =>
      ["captured", "succeeded"].includes(p.status)
    );

    const revenue = capturedPayments.reduce(
      (sum, p) => sum + Number(p.amount || 0),
      0
    );

    const tips = capturedPayments
      .filter((p) => p.type === "tip")
      .reduce((sum, p) => sum + Number(p.amount || 0), 0);
    ...
    return ok(res, {
      analytics: {
        ...
        onlineDrivers: (drivers || []).filter((d) => d.available === true).length,
        totalRides: rides?.length || 0,
        completedRides: completedRides.length,
        canceledRides: (rides || []).filter((r) => r.status === "canceled").length,
        totalRevenue: roundMoney(revenue),
        totalTips: roundMoney(tips),
        averageCompletedFare: completedRides.length
          ? roundMoney(
              completedRides.reduce((sum, r) => sum + Number(r.fare_total || 0), 0) /
    ...
    return serverError(res, error, "Could not load analytics.");
  }
});

/* =========================================================
   PART 8 END
   NEXT: PART 9 — STRIPE WEBHOOK + AI SUPPORT + STARTUP CHECKS + SERVER LISTEN
========================================================= *//* =========================================================
   HARVEY TAXI — PRODUCTION CODE BLUE SERVER.JS
   PART 9 OF 9
   STRIPE WEBHOOK + AI SUPPORT + STARTUP CHECKS
   DISPATCH SWEEP + ERROR HANDLER + SERVER LISTEN
========================================================= */

/* =========================================================
   IMPORTANT STRIPE WEBHOOK NOTE
   In your FINAL assembled server.js, this Stripe webhook route
  ...
  async (req, res) => {
    try {
      ...
      if (event.type === "payment_intent.requires_capture") {
        ...
        await auditLog("stripe_payment_requires_capture", {
      ...
      if (event.type === "payment_intent.succeeded") {
        ...
        if (payment) {
          const updatedPayment = await dbUpdateById("payments", payment.id, {
            status: "succeeded",
            stripe_latest_status: "succeeded",
            captured_at: nowIso(),
            captured_amount: roundMoney((object.amount_received || object.amount || 0) / 100),
          });
          ...
          await auditLog("stripe_payment_succeeded", {
      ...
      if (event.type === "payment_intent.payment_failed") {
        ...
        await auditLog("stripe_payment_failed", {
          stripe_payment_intent_id: object.id,
          failure_message: failureMessage,
        });
      }
      ...
      if (event.type === "payment_intent.canceled") {
        ...
        await auditLog("stripe_payment_canceled", {
          stripe_payment_intent_id: object.id,
        });
      }
...
app.post("/api/ai/support", async (req, res) => {
  try {
    ...
    if (!openai) {
      return ok(res, {
        reply:
          "Harvey Taxi AI support is not available right now. Please contact support at williebee@harveytaxiservice.com.",
      });
    }
...
Business:
- Harvey Taxi Service LLC is a transportation platform.
- The platform supports rider signup, driver onboarding, driver verification, payment authorization, ride dispatch, trip status, tips, and support.
- There may be an Autonomous Pilot mode, but it should be described as pilot/future-ready, not guaranteed autonomous service.

Rules:
- Be clear, calm, professional, and helpful.
- If the user mentions emergency, danger, injury, violence, or medical emergency, tell them to call 911 immediately.
- Do not claim a ride, payment, approval, refund, or background check was completed unless the system data confirms it.
- For driver onboarding: explain email verification, phone verification if enabled, Checkr background check, and admin approval.
- For rider flow: explain rider approval and payment authorization before dispatch.
- For payments: explain authorization is a hold before the ride and capture happens after completion.
...
`;
    ...
    await auditLog("ai_support_message", {
      page,
...
app.post("/api/support/contact", async (req, res) => {
  try {
    ...
    const email = normalizeEmail(req.body.email);
    const message = normalizeText(req.body.message);
    const topic = normalizeText(req.body.topic || "General Support");
    ...
    await sendEmail({
      to: SUPPORT_EMAIL,
      subject: `Harvey Taxi Support: ${topic}`,
      text: `Name: ${name}\nEmail: ${email}\nTopic: ${topic}\n\n${message}`,
      html: `
        <div style="font-family:Arial,sans-serif;">
          <h2>Harvey Taxi Support Request</h2>
    ...
    await auditLog("support_contact_submitted", {
      name,
      email,
      topic,
    });
    ...
    return serverError(res, error, "Support request failed.");
  }
});
...
async function runStartupChecks() {
  console.log("🚀 Harvey Taxi startup checks running...");
  ...
  for (const table of requiredTables) {
    try {
      ...
      console.warn(`⚠️ Table check crashed: ${table} — ${error.message}`);
    }
  }
...
server.listen(PORT, async () => {
  console.log("=================================================");
  console.log("🚕 HARVEY TAXI — PRODUCTION CODE BLUE SERVER");
  console.log("=================================================");
...
/* =========================================================
   END OF PRODUCTION CODE BLUE SERVER.JS
========================================================= */
