...
function publicVehicleStage(ride, vehicle, gpsPoint) {
  if (!ride) {
    return {
      stage: "searching",
      progress: 15,
  ...
  if (ride.status === "completed") {
    return {
      ...
      progress: 100,
      etaMinutes: 0,
      vehicleLabel: vehicle?.name || vehicle?.vehicle || "Vehicle",
...
app.post("/api/rider/signup", async (req, res) => {
  try {
    ...
    if (PERSONA_API_KEY && PERSONA_RIDER_TEMPLATE_ID) {
      ...
      rider.personaUrl = inquiry.attributes?.inquiry_url || inquiry.attributes?.inquiry_link || null
    }
...
app.post("/api/driver/signup", async (req, res) => {
  try {
    ...
    if (PERSONA_API_KEY && PERSONA_DRIVER_TEMPLATE_ID) {
      ...
      driver.personaUrl = inquiry.attributes?.inquiry_url || inquiry.attributes?.inquiry_link || null
    }
...
app.post("/api/request-ride", (req, res) => {
  ...
  // internal-only pickup coords allowed for backend math
  ride.pickupLat = typeof req.body.pickupLat === "number" ? req.body.pickupLat : null
  ride.pickupLng = typeof req.body.pickupLng === "number" ? req.body.pickupLng : null
  ride.dropoffLat = typeof req.body.dropoffLat === "number" ? req.body.dropoffLat : null
  ride.dropoffLng = typeof req.body.dropoffLng === "number" ? req.body.dropoffLng : null
...
app.post("/api/driver/claim-ride", (req, res) => {
  ...
  ride.driverName = driver.name || `${driver.firstName || ""} ${driver.lastName || ""}`.trim()
...
// driver/device sends live location
...
// rider-safe tracking view: no raw coords returned
app.get("/api/ride/:rideId/live-status", (req, res) => {
  ...
  const driver =
    drivers.find(d => String(d.id) === String(ride.driverId || ride.vehicleId)) || null

  const gpsPoint =
    gps.find(g => String(g.driverId) === String(ride.driverId || ride.vehicleId)) || null
...
})process.env.CHECKR_WORK_STATE || "TN"
const CHECKR_WORK_CITY = process.env.CHECKR_WORK_CITY || "Nashville"
...
function publicVehicleStage(ride, driver, gpsPoint) {
  ...
  if (ride.status === "completed") {
    return {
      ...
      progress: 100,
      etaMinutes: 0,
      vehicleLabel: driver?.name || "Vehicle",
      liveLabel: "Ride completed"
    }
  }
