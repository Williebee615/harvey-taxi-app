...
app.post("/api/rider/signup", async (req, res) => {
  try {
    ...
    await createAdminLog("rider_signup", { rider_id: riderId });
...
app.post("/api/riders/:riderId/approve", async (req, res) => {
  try {
...
app.post("/api/driver/signup", async (req, res) => {
  try {
    ...
    await createAdminLog("driver_signup", { driver_id: driverId });
...
app.post("/api/drivers/:driverId/approve", async (req, res) => {
  try {
  ...
  try {
    ...
    if (!["available", "offline", "busy"].includes(availability)) {
      return res.status(400).json({
        ok: false,
        error: "Availability must be available, offline, or busy"
      });
    }
...
app.get("/api/driver/:driverId/missions", async (req, res) => {
  try {
    ...
    return res.json({ ok: true, missions: data || [] });
  } catch (error) {
...
app.post("/api/driver/:driverId/accept-ride", async (req, res) => {
  try {
    ...
    const openDispatch = await getOpenDispatchByRideId(ride.ride_id);
    if (!openDispatch) {
      return res.status(404).json({ ok: false, error: "No active dispatch found" });
    }

    if (openDispatch.driver_id !== driverId) {
      return res.status(403).json({
        ok: false,
        error: "This mission is not assigned to this driver"
      });
    }

    const isExpired =
      openDispatch.expires_at && new Date(openDispatch.expires_at).getTime() < Date.now();
...
app.post("/api/driver/:driverId/decline-ride", async (req, res) => {
  try {
    ...
    const openDispatch = await getOpenDispatchByRideId(ride.ride_id);
    if (!openDispatch || openDispatch.driver_id !== driverId) {
      return res.status(404).json({ ok: false, error: "Active dispatch not found for driver" });
    }
...
app.get("/api/admin/analytics", async (req, res) => {
  try {
    const [{ count: ridersCount }, { count: driversCount }, { count: ridesCount }] =
      await Promise.all([
