...
const supabase = createClient(SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY, {
  ...
  auth: {
   ...
   SECURITY, MIDDLEWARE, HELPERS, RESPONSE UTILITIES
...
const RAW_WEBHOOK_LIMIT =

  env("RAW_WEBHOOK_LIMIT", "2mb");
...
function calculateDeliveryEstimate({
  ...
  const smallOrderFee =
    ...
    safeSubtotal > 0 && safeSubtotal < DELIVERY_SMALL_ORDER_THRESHOLD
  ...
  const multiplier =
    ...
    delivery_type === "grocery"
      ...
      ? GROCERY_DELIVERY_MULTIPLIER

      : FOOD_DELIVERY_MULTIPLIER;
  ...
  return {
   ...
   HTAF FOUNDATION APPLICATION SYSTEM
...
async function createHTAFApplication(payload) {
  ...
  const application = {
    ...
    first_name:
      ...
      cleanString(payload.first_name, 120),

    last_name:

      cleanString(payload.last_name, 120),
    ...
    county:
      ...
      cleanString(payload.county, 120),

    city:

      cleanString(payload.city, 120),

    applicant_type:

      cleanString(payload.applicant_type, 80),

    household_size:

      Number(payload.household_size || 0),

    monthly_income:

      Number(payload.monthly_income || 0),
    ...
    pickup_city:
      ...
      cleanString(payload.pickup_city, 150),

    destination:

      cleanString(payload.destination, 255),
...
async function sendAdminApplicationAlert(
    ...
    await sendEmail({
      ...
      html: `
        ...
        <h2>
          ...
          New HTAF Application Submitted
...
app.patch(
  ...
  asyncRoute(async (req, res) => {
   ...
   AUTH, RIDER/DRIVER ONBOARDING, EMAIL + SMS VERIFY
...
async function verifyCode({
  ...
  if (Number(data.attempts || 0) >= Number(data.max_attempts || 5)) {
...
app.post(
  ...
  asyncRoute(async (req, res) => {
    ...
    const rider = {
      ...
      status: ENABLE_RIDER_APPROVAL_GATE
    ...
    return ok(res, {
      ...
      next_steps: {
        ...
        persona_verification: ENABLE_PERSONA
  ...
  asyncRoute(async (req, res) => {
    ...
    return ok(res, {
      ...
      next_steps: {
        ...
        background_check: ENABLE_CHECKR
  ...
  asyncRoute(async (req, res) => {
    ...
    const result =
      ...
      await verifyCode({
        ...
        purpose: cleanString(req.body.purpose, 80)
  ...
  asyncRoute(async (req, res) => {
    ...
    await sendSms({
      ...
      body:
        ...
        `Your Harvey Taxi verification code is ${code}. ` +
  ...
  asyncRoute(async (req, res) => {
    ...
    const result =
      ...
      await verifyCode({
        ...
        code: cleanString(req.body.code, 20),
   ...
   PERSONA IDENTITY VERIFICATION + CHECKR BACKGROUND CHECKS
...
async function personaRequest(pathname, body = {}) {
  ...
  const response =
    ...
    await fetch(`https://withpersona.com/api/v1${pathname}`, {
...
app.post(
  ...
  asyncRoute(async (req, res) => {
    ...
    const userType =
      ...
      cleanString(req.body.user_type, 20).toLowerCase();
    ...
    await supabase
      ...
      .update({
        ...
        persona_inquiry_id: inquiry?.id || null,
...
app.post(
  ...
  asyncRoute(async (req, res) => {
    ...
    try {
      ...
      return fail(res, "Invalid Persona webhook payload.", 400);

    }

    if (!verifyPersonaSignature(req)) {

      return fail(res, "Invalid Persona signature.", 401);
    ...
    const referenceId =
      ...
      inquiry?.relationships?.account?.data?.id ||
    ...
    await auditLog({
      ...
      action: "persona_webhook_received",
...
async function checkrRequest(pathname, body = {}) {
  ...
  const response =
    ...
    await fetch(`https://api.checkr.com/v1${pathname}`, {
...
app.post(
  ...
  asyncRoute(async (req, res) => {
    ...
    const invitation =
      ...
      await createCheckrInvitation(candidate.id);
...
app.post(
  ...
  asyncRoute(async (req, res) => {
    ...
    try {
      ...
      return fail(res, "Invalid Checkr webhook payload.", 400);

    }

    if (!verifyCheckrSignature(req)) {

      return fail(res, "Invalid Checkr signature.", 401);
    ...
    const clear =
      ...
      ["clear", "complete", "completed"].includes(
    ...
    if (candidateId) {
      ...
      await supabase
        ...
        .update({
          ...
          approval_status: clear ? "eligible_for_review" : "pending",
    ...
    await auditLog({
      ...
      action: "checkr_webhook_received",
...
app.patch(
  ...
  asyncRoute(async (req, res) => {
    ...
    const reason =
      ...
      cleanString(req.body.reason, 1000);
   ...
   RIDE ESTIMATES, PAYMENT INTENTS, RIDE REQUESTS, DISPATCH
...
async function getRiderReadiness(riderId) {
  ...
  const checks = {
    ...
    persona_verified: ENABLE_PERSONA

      ? Boolean(rider.persona_verified)

      : true,

    status_ready: ["active", "approved"].includes(
...
async function findAvailableDrivers({
  ...
  radius_miles = DRIVER_SEARCH_RADIUS_MILES,
    ...
    (data || [])
      ...
      .map((driver) => {
        ...
        const lat =
          ...
          Number(driver.current_lat || driver.latitude);

        const lng =

          Number(driver.current_lng || driver.longitude);
...
async function dispatchRide(ride) {
  ...
  await supabase
    ...
    .update({
      ...
      current_driver_id: firstDriver.id,
...
app.post(
  ...
  asyncRoute(async (req, res) => {
    ...
    const miles =
      ...
      Number(req.body.miles || req.body.distance_miles || 0);

    const minutes =

      Number(req.body.minutes || req.body.duration_minutes || 0);

    const rideType =

      normalizeRideType(req.body.ride_type);
  ...
  asyncRoute(async (req, res) => {
    ...
    const estimate =
      ...
      calculateRideEstimate({
        ...
        ride_type: normalizeRideType(req.body.ride_type)
    ...
    const paymentIntent =
      ...
      await stripe.paymentIntents.create({
        ...
        metadata: {
          ...
          rider_id: cleanString(req.body.rider_id, 100)
    ...
    await auditLog({
      ...
      action: "ride_payment_intent_created",

      actor_type: "rider",

      actor_id: cleanString(req.body.rider_id, 100),
  ...
  asyncRoute(async (req, res) => {
    ...
    const estimate =
      ...
      calculateRideEstimate({
        ...
        miles: Number(req.body.miles || req.body.distance_miles || 0),

        minutes: Number(req.body.minutes || req.body.duration_minutes || 0),
    ...
    const ride = {
      ...
      dispatch_status: status === RIDE_STATUS.PAYMENT_REQUIRED
...
/* =========================================================
   ...
   AUTHORIZE EXISTING RIDE AFTER PAYMENT
  ...
  asyncRoute(async (req, res) => {
    ...
    const paymentIntentId =
      ...
      cleanString(
        ...
        req.body.payment_intent_id || ride.payment_intent_id,
    ...
    const dispatch =
      ...
      await dispatchRide(updatedRide);
...
app.post(
  ...
  asyncRoute(async (req, res) => {
    ...
    const offerId =
      ...
      cleanString(req.params.offerId, 100);
    ...
    if (offer.status !== "pending") {
      ...
      return fail(res, "Offer is no longer available.", 409);

    }

    if (driverId && offer.driver_id !== driverId) {

      return fail(res, "Offer does not belong to this driver.", 403);
    ...
    await supabase
      ...
      .update({
        ...
        current_driver_id: offer.driver_id,
        ...
        if (dispatchAttempts < MAX_DISPATCH_ATTEMPTS) {
          ...
          await supabase
            ...
            .update({
              ...
              dispatch_attempts: dispatchAttempts + 1,
...
app.post(
  ...
  asyncRoute(async (req, res) => {
    ...
    const rideId =
      ...
      cleanString(req.params.rideId, 100);
...
async function captureRidePayment(
...
app.get(
  ...
  asyncRoute(async (req, res) => {
    ...
    const driverId =
      ...
      cleanString(req.params.driverId, 100);
...
app.get(
  ...
  asyncRoute(async (req, res) => {
   ...
   ADMIN OPERATIONS, HTAF RIDE MANAGEMENT, SSE LIVE STREAMS
  ...
  asyncRoute(async (req, res) => {
    ...
    return ok(res, {
      ...
      overview: {
        ...
        htaf_applications: htaf.count || 0,
...
app.get(
  ...
  asyncRoute(async (req, res) => {
    ...
    const status =
      ...
      cleanString(req.query.status, 80);
...
app.patch(
  ...
  asyncRoute(async (req, res) => {
    ...
    const status =
      ...
      cleanString(req.body.status, 80);
      ...
      await supabase
        ...
        .update({
          ...
          admin_note:
            ...
            cleanString(req.body.note, 1000),
    ...
    await auditLog({
      ...
      action: "admin_ride_status_updated",
    ...
    const {
      ...
      await supabase
        ...
        .update({
          ...
          dispatch_status: "admin_assigned",
...
/* =========================================================
   ...
   HTAF APPROVE APPLICATION INTO RIDE
  ...
  asyncRoute(async (req, res) => {
    ...
    const ride = {
      ...
      pickup:
        ...
        cleanString(req.body.pickup || application.pickup_city, 500),

      destination:

        cleanString(req.body.destination || application.destination, 500),
    ...
    await auditLog({
      ...
      action: "htaf_application_converted_to_ride",
    ...
    broadcastSse("htaf_ride_created", {
...
async function getDriverCompliance(
...
/* =========================================================
   ...
   BLOCK NON COMPLIANT DRIVER

========================================================= */

async function requireCompliantDriver(
  ...
  if (!compliance.eligible) {
    ...
    throw new Error(
      ...
      "Driver compliance requirements not met."
...
app.post(
  ...
  asyncRoute(async (req, res) => {
    ...
    const verifyUrl =
      ...
      `${APP_URL}/verify-email.html?token=${token}`;
...
async function updateDriverRisk(
...
app.get(
  ...
  asyncRoute(async (req, res) => {
   ...
   STRIPE WEBHOOKS, HEALTH, MONITORING, PRODUCTION BOOTSTRAP
...
app.post(
  ...
  asyncRoute(async (req, res) => {
    ...
    if (!stripe || !STRIPE_WEBHOOK_SECRET) {
      ...
      return fail(res, "Stripe webhook not configured.", 503);
    ...
    if (event.type === "payment_intent.succeeded") {
      ...
      await supabase
        ...
        .eq("payment_intent_id", object.id);
...
app.get(
  ...
  asyncRoute(async (req, res) => {
    ...
    return ok(res, {
      ...
      dispatch_paused: dispatchPaused === "true",
...
app.post(
  ...
  asyncRoute(async (req, res) => {
    ...
    const message =
      ...
      cleanString(req.body.message, 4000);

    const page =

      cleanString(req.body.page, 120);
        ...
        messages: [

          {
            ...
            content:
              ...
              "You are Harvey Taxi and Harvey Transportation Assistance Foundation support. Help riders, drivers, applicants, and donors with clear, safe, concise guidance. Do not promise guaranteed assistance, approval, payment, or emergency response."
...
app.get("/foundation", (req, res) => {
  ...
  res.sendFile(
    ...
    path.join(PUBLIC_DIR, "foundation.html")

  );

});

app.get("/htaf-application", (req, res) => {

  res.sendFile(

    path.join(PUBLIC_DIR, "htaf-application.html")
...
app.get("/request-ride", (req, res) => {
  ...
  res.sendFile(
    ...
    path.join(PUBLIC_DIR, "request-ride.html")
...
app.get("/support", (req, res) => {
  ...
  res.sendFile(
    ...
    path.join(PUBLIC_DIR, "support.html")
