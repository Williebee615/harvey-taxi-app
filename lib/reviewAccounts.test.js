const {
  ...
  resolveReviewLoginClientMessage,
...
describe("resolveReviewLoginClientMessage (follow-up fix: 429 must never look like a wrong password)", () => {
  test("503 still reports the flag-disabled message", () => {
    expect(resolveReviewLoginClientMessage({ status: 503, retryAfterSeconds: null })).toBe(
      "Google Play review sign-in is temporarily disabled."
    );
  });

  test("429 with no retry_after_seconds falls back to the exact requested wording", () => {
    expect(resolveReviewLoginClientMessage({ status: 429, retryAfterSeconds: null })).toBe(
      "Too many sign-in attempts. Wait 10 minutes and try again."
    );
  });

  test("429 with a retry_after_seconds honors it, rounding up to whole minutes", () => {
    expect(resolveReviewLoginClientMessage({ status: 429, retryAfterSeconds: 61 })).toBe(
      "Too many sign-in attempts. Wait 2 minutes and try again."
    );
    expect(resolveReviewLoginClientMessage({ status: 429, retryAfterSeconds: 60 })).toBe(
      "Too many sign-in attempts. Wait 1 minute and try again."
    );
  });

  test("429 with a zero or negative retry_after_seconds falls back to the generic wording rather than saying 'wait 0 minutes'", () => {
    expect(resolveReviewLoginClientMessage({ status: 429, retryAfterSeconds: 0 })).toBe(
      "Too many sign-in attempts. Wait 10 minutes and try again."
    );
    expect(resolveReviewLoginClientMessage({ status: 429, retryAfterSeconds: -5 })).toBe(
      "Too many sign-in attempts. Wait 10 minutes and try again."
    );
  });

  test("401 (wrong password) and 401 (account not found) both produce the identical generic message -- no enumeration signal", () => {
    const wrongPassword = resolveReviewLoginClientMessage({ status: 401, retryAfterSeconds: null });
    const noSuchAccount = resolveReviewLoginClientMessage({ status: 401, retryAfterSeconds: null });
    expect(wrongPassword).toBe("Invalid reviewer credentials.");
    expect(wrongPassword).toBe(noSuchAccount);
  });

  test("any other unexpected status also falls back to the generic message rather than throwing", () => {
    expect(resolveReviewLoginClientMessage({ status: 500, retryAfterSeconds: null })).toBe(
      "Invalid reviewer credentials."
    );
    expect(resolveReviewLoginClientMessage({ status: 0, retryAfterSeconds: null })).toBe(
      "Invalid reviewer credentials."
    );
  });
});
