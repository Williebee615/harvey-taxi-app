...
// Idempotency for POST /api/checkr/start: a driver clicking "Start
// Background Check" twice (double-click, two tabs, retry after a slow
// network response) must not create a second Checkr candidate and
// invitation. Decides, from the driver's current row, whether to reuse
// the existing invitation, report the check as already done, or actually
// call Checkr to create a new one.
function decideCheckrStartAction(driver) {
  const status = String(driver?.checkr_status || "").toLowerCase();

  if (CHECKR_CLEAR_STATUSES.has(status)) {
    return { action: "already_clear", checkr_status: driver?.checkr_status || null };
  }

  if (driver?.checkr_invitation_id && driver?.checkr_invitation_url) {
    return { action: "reuse", invitation_url: driver.checkr_invitation_url };
  }

  return { action: "create" };
}

// Same idempotency rule for POST /api/persona/inquiry on the driver
// path: reuse the existing inquiry rather than creating a new one on a
// repeat click.
function decidePersonaStartAction(driver) {
  const status = String(driver?.persona_status || "").toLowerCase();

  if (PERSONA_APPROVED_STATUSES.has(status)) {
    return { action: "already_verified", persona_status: driver?.persona_status || null };
  }

  if (driver?.persona_inquiry_id) {
    return {
      action: "reuse",
      inquiry_id: driver.persona_inquiry_id,
