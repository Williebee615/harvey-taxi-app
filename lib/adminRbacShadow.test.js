...
describe("findAdminRoleRow() -- case-insensitive email identity matching", () => {
  test("CaseWorker@HarveyTaxiService.com and caseworker@harveytaxiservice.com resolve to the same admin role", () => {
    const rows = [{ email: "CaseWorker@HarveyTaxiService.com", role: "htaf_caseworker" }];
    expect(findAdminRoleRow(rows, "caseworker@harveytaxiservice.com")).toEqual(rows[0]);
    expect(findAdminRoleRow(rows, "CaseWorker@HarveyTaxiService.com")).toEqual(rows[0]);
    expect(findAdminRoleRow(rows, "CASEWORKER@HARVEYTAXISERVICE.COM")).toEqual(rows[0]);
  });

  test("matches regardless of which side (stored row vs. lookup target) is normalized", () => {
    const mixedCaseRow = [{ email: "Admin@Example.com", role: "super_admin" }];
    const lowercaseRow = [{ email: "admin@example.com", role: "super_admin" }];
    expect(findAdminRoleRow(mixedCaseRow, "admin@example.com")).toEqual(mixedCaseRow[0]);
    expect(findAdminRoleRow(lowercaseRow, "Admin@Example.com")).toEqual(lowercaseRow[0]);
  });

  test("no match returns null, not undefined, and never throws", () => {
    expect(findAdminRoleRow([], "someone@example.com")).toBeNull();
    expect(findAdminRoleRow([{ email: "a@example.com", role: "support" }], "b@example.com")).toBeNull();
    expect(findAdminRoleRow(null, "a@example.com")).toBeNull();
    expect(findAdminRoleRow(undefined, "a@example.com")).toBeNull();
    expect(() => findAdminRoleRow(undefined, "a@example.com")).not.toThrow();
  });

  test("whitespace around the lookup email is trimmed before matching", () => {
    const rows = [{ email: "admin@example.com", role: "super_admin" }];
    expect(findAdminRoleRow(rows, "  admin@example.com  ")).toEqual(rows[0]);
  });

  test("a row with a missing or non-string email is skipped, never matches, and never throws", () => {
    const rows = [{ email: null, role: "support" }, { role: "finance" }, {}, { email: "real@example.com", role: "dispatcher" }];
    expect(() => findAdminRoleRow(rows, "real@example.com")).not.toThrow();
    expect(findAdminRoleRow(rows, "real@example.com")).toEqual(rows[3]);
    expect(findAdminRoleRow(rows, "")).toBeNull();
  });
});
