...
// payload. The AI is not provided direct identifiers, exact financial
...
// characteristics. Household size and monthly income (exact OR banded)
// are excluded entirely, not coarsened -- once triage is limited to
// operational categorization and workflow prioritization rather than
// eligibility, income and household composition have no necessary
// function in that payload, and even a coarse band still exposes
// socioeconomic information and invites the model to reason about
// financial circumstances. Field-by-field classification is documented
// in the file above; this is the implementation of that
// classification, not a restatement of it.
// ============================================================
...
// derived/boolean-ized from a sensitive source field that is itself
// never included. Household size and monthly income are excluded
// entirely -- not banded -- since triage is limited to operational
// categorization/workflow prioritization, not eligibility, and neither
// field has a necessary function in that scope; even a coarse band
// still exposes socioeconomic information and invites the model to
// reason about financial circumstances. Called server-side only
// (server.js's triageHtafApplication); the client never sees or
// supplies this payload.
