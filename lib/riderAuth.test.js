...
describe("buildRiderVerificationFieldUpdate — marking phone/email verified on a successful login OTP (P0 remediation, PR #2a)", () => {
  test("a successful phone OTP marks sms_verified true, when not already true", () => {
    const update = buildRiderVerificationFieldUpdate({
      channel: "phone",
      rider: { sms_verified: false, email_verified: false }
    });

    expect(update).toEqual({ sms_verified: true });
  });

  test("a successful email OTP marks email_verified true, when not already true", () => {
    const update = buildRiderVerificationFieldUpdate({
      channel: "email",
      rider: { sms_verified: false, email_verified: false }
    });

    expect(update).toEqual({ email_verified: true });
  });

  test("a phone OTP never also marks email_verified, even if email is currently unverified", () => {
    const update = buildRiderVerificationFieldUpdate({
      channel: "phone",
      rider: { sms_verified: false, email_verified: false }
    });
  ...
  test("an email OTP never also marks sms_verified", () => {
    const update = buildRiderVerificationFieldUpdate({
      channel: "email",
      rider: { sms_verified: false, email_verified: false }
    });

    expect(update).not.toHaveProperty("sms_verified");
  });

  test("returns null (no write needed) when the channel's flag is already true", () => {
    const phoneAlready = buildRiderVerificationFieldUpdate({
      channel: "phone",
      rider: { sms_verified: true, email_verified: false }
    });
    const emailAlready = buildRiderVerificationFieldUpdate({
      channel: "email",
      rider: { sms_verified: false, email_verified: true }
    });

    expect(phoneAlready).toBeNull();
    expect(emailAlready).toBeNull();
  });

  test("a rider row with no verification columns at all (undefined, not false) still returns an update, not null", () => {
    const update = buildRiderVerificationFieldUpdate({ channel: "phone", rider: {} });

    expect(update).toEqual({ sms_verified: true });
  });
});

describe("buildAuthUiConfigResponse — GET /api/rider/auth-ui-config's response contract (P0 remediation, PR #2a)", () => {
  test("returns exactly one key: enabled", () => {
    const result = buildAuthUiConfigResponse({ enabled: true });

    expect(Object.keys(result)).toEqual(["enabled"]);
  });

  test("passes through a real true", () => {
    expect(buildAuthUiConfigResponse({ enabled: true })).toEqual({ enabled: true });
  });

  test("false, undefined, and non-boolean truthy values all coerce to false -- only a literal true is ever enabled", () => {
    expect(buildAuthUiConfigResponse({ enabled: false })).toEqual({ enabled: false });
    expect(buildAuthUiConfigResponse({ enabled: undefined })).toEqual({ enabled: false });
    expect(buildAuthUiConfigResponse({ enabled: "true" })).toEqual({ enabled: false });
    expect(buildAuthUiConfigResponse({ enabled: 1 })).toEqual({ enabled: false });
  });
});

describe("resolveEnforcedRiderId — the IDOR fix itself (P0 remediation, PR #2b)", () => {
  test("an authenticated identity always wins over a client-supplied one, even when they differ", () => {
    const result = resolveEnforcedRiderId({
      authenticatedRiderId: "RIDER-VICTIM",
      clientSuppliedRiderId: "RIDER-ATTACKER"
    });

    expect(result).toBe("RIDER-VICTIM");
  });

  test("an authenticated identity wins even when no client-supplied value was sent at all", () => {
    const result = resolveEnforcedRiderId({
      authenticatedRiderId: "RIDER-1",
      clientSuppliedRiderId: undefined
    });

    expect(result).toBe("RIDER-1");
  });

  test("falls back to the client-supplied value only when there is no authenticated identity (flag off / not yet enforced)", () => {
    const result = resolveEnforcedRiderId({
      authenticatedRiderId: null,
      clientSuppliedRiderId: "RIDER-2"
    });

    expect(result).toBe("RIDER-2");
  });

  test("no authenticated identity and no client-supplied value returns an empty string, never undefined/null", () => {
    const result = resolveEnforcedRiderId({
      authenticatedRiderId: null,
      clientSuppliedRiderId: null
    });

    expect(result).toBe("");
  });

  test("an empty-string authenticated id (should never happen, but defensively) is treated as absent, not as a valid identity", () => {
    const result = resolveEnforcedRiderId({
      authenticatedRiderId: "",
      clientSuppliedRiderId: "RIDER-3"
    });
...
describe("resolveEnforcedRiderId — applied to the P0-2 payment routes (P0 remediation, PR #3)", () => {
  // These four scenarios name the exact IDOR this PR closes on
  // POST /api/rider/payment-methods/setup-intent, GET /api/rider/payment-methods,
  // DELETE /api/rider/payment-methods/:paymentMethodId, and POST
  // /api/rides/payment-intent (server.js) -- each route resolves its
  // riderId through this same function rather than trusting
  // req.body.rider_id/req.query.riderId directly. The underlying
  // decision is already exhaustively covered above; these are kept as
  // their own named block so a payment-specific regression reads
  // clearly as "the payment routes stopped doing this" rather than only
  // as a generic resolveEnforcedRiderId assertion.
  test("a rider's own authenticated session always wins over a spoofed rider_id/riderId aimed at another rider's payment methods or Stripe customer", () => {
    const result = resolveEnforcedRiderId({
      authenticatedRiderId: "RIDER-SELF",
      clientSuppliedRiderId: "RIDER-VICTIM-STRIPE-CUSTOMER"
    });

    expect(result).toBe("RIDER-SELF");
  });

  test("while rider_auth_enforced is off (no authenticated identity yet), the still-open P0-2 gap is unchanged: the client-supplied rider_id is used exactly as before this PR", () => {
    const result = resolveEnforcedRiderId({
      authenticatedRiderId: undefined,
      clientSuppliedRiderId: "RIDER-CLIENT-SUPPLIED"
    });

    expect(result).toBe("RIDER-CLIENT-SUPPLIED");
  });

  test("no session and no client-supplied rider_id yields an empty string, which every migrated payment route already treats as 'rider_id/riderId is required' (400), never as an implicit match", () => {
    const result = resolveEnforcedRiderId({
      authenticatedRiderId: undefined,
      clientSuppliedRiderId: undefined
    });
...
describe("resolveEnforcedRiderId — applied to POST /api/rides/request (ride-creation ownership)", () => {
  // ride.rider_id (set here, at creation) is the value
  // verifyPaymentIntentForRide()'s rider-match check compares the
  // PaymentIntent's own metadata.rider_id (fixed by the payment-route
  // work, lib/riderPayments.test.js's "payment-route IDOR/cross-account
  // composition" suite) against -- until this route resolves its own
  // riderId the same way, that comparison has a trustworthy side and an
  // untrustworthy one. This block names that closure explicitly against
  // this specific route, mirroring the payment-route describe block
  // above rather than relying on a reader to infer it from the generic
  // coverage alone.
  test("a rider's own authenticated session always wins over a spoofed rider_id aimed at creating a ride that appears to belong to someone else", () => {
    const result = resolveEnforcedRiderId({
      authenticatedRiderId: "RIDER-SELF",
      clientSuppliedRiderId: "RIDER-VICTIM"
    });

    expect(result).toBe("RIDER-SELF");
  });

  test("while rider_auth_enforced is off (no authenticated identity yet), the client-supplied rider_id is used exactly as before -- the still-open gap this closes once enforcement is on", () => {
    const result = resolveEnforcedRiderId({
      authenticatedRiderId: undefined,
      clientSuppliedRiderId: "RIDER-CLIENT-SUPPLIED"
    });
  ...
  test("no session and no client-supplied rider_id yields an empty string -- the route creates the ride with rider_id: null rather than inventing an identity", () => {
    const result = resolveEnforcedRiderId({
      authenticatedRiderId: undefined,
      clientSuppliedRiderId: undefined
    });
