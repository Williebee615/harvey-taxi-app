...
// RENDER_EXTERNAL_HOSTNAME fix: production evidence (a rejected-origin
// audit row, captured via buildCorsRejectionError's own diagnostics)
// showed a real same-origin request arriving with
// Origin: https://harvey-taxi-app-2.onrender.com even though the page
// was loaded on the custom domain -- and that exact origin was rejected
// because APP_BASE_URL is pinned to https://harveytaxiservice.com in
// production, so Render's own assigned hostname no longer appeared
// anywhere in the default allow-list. This adds exactly that one
// service's own hostname (from Render's auto-populated
// RENDER_EXTERNAL_HOSTNAME env var), a bare hostname -- never a
// wildcard, so no other onrender.com service is ever accepted.
describe("isAllowedOrigin / buildDefaultAllowedOrigins — RENDER_EXTERNAL_HOSTNAME", () => {
  const RENDER_HOSTNAME = "harvey-taxi-app-2.onrender.com";

  it("allows this exact service's own Render-assigned origin", () => {
    const config = { ...BASE_CONFIG, renderExternalHostname: RENDER_HOSTNAME };
    expect(isAllowedOrigin(`https://${RENDER_HOSTNAME}`, config)).toBe(true);
  });

  it("does not allow an unrelated onrender.com service -- no wildcard", () => {
    const config = { ...BASE_CONFIG, renderExternalHostname: RENDER_HOSTNAME };
    expect(isAllowedOrigin("https://some-other-app.onrender.com", config)).toBe(false);
    expect(isAllowedOrigin("https://evil.onrender.com", config)).toBe(false);
  });

  it("still allows the canonical and foundation apex/www origins alongside the Render origin", () => {
    const config = { ...BASE_CONFIG, renderExternalHostname: RENDER_HOSTNAME };
    expect(isAllowedOrigin("https://harveytaxiservice.com", config)).toBe(true);
    expect(isAllowedOrigin("https://www.harveytaxiservice.com", config)).toBe(true);
    expect(isAllowedOrigin("https://harveytransportationfoundation.com", config)).toBe(true);
    expect(isAllowedOrigin("https://www.harveytransportationfoundation.com", config)).toBe(true);
  });

  it("ignores a missing/empty RENDER_EXTERNAL_HOSTNAME without throwing or allowing anything extra", () => {
    expect(() => isAllowedOrigin(`https://${RENDER_HOSTNAME}`, { ...BASE_CONFIG, renderExternalHostname: "" })).not.toThrow();
    expect(isAllowedOrigin(`https://${RENDER_HOSTNAME}`, { ...BASE_CONFIG, renderExternalHostname: "" })).toBe(false);
    expect(isAllowedOrigin(`https://${RENDER_HOSTNAME}`, { ...BASE_CONFIG, renderExternalHostname: undefined })).toBe(false);
  });

  it("safely ignores a malformed RENDER_EXTERNAL_HOSTNAME (a full URL instead of a bare hostname) without throwing", () => {
    const config = { ...BASE_CONFIG, renderExternalHostname: `https://${RENDER_HOSTNAME}` };
    expect(() => isAllowedOrigin(`https://${RENDER_HOSTNAME}`, config)).not.toThrow();
    // A scheme-carrying value never becomes a trusted origin -- it's
    // dropped entirely rather than double-prefixed into something like
    // "https://https://harvey-taxi-app-2.onrender.com".
    expect(isAllowedOrigin(`https://${RENDER_HOSTNAME}`, config)).toBe(false);
    expect(isAllowedOrigin(`https://https://${RENDER_HOSTNAME}`, config)).toBe(false);
    // Nothing else regresses because of the malformed value.
    expect(isAllowedOrigin("https://harveytaxiservice.com", config)).toBe(true);
  });

  it("safely ignores a RENDER_EXTERNAL_HOSTNAME containing a path or whitespace", () => {
    expect(isAllowedOrigin(`https://${RENDER_HOSTNAME}`, { ...BASE_CONFIG, renderExternalHostname: `${RENDER_HOSTNAME}/path` })).toBe(false);
    expect(isAllowedOrigin(`https://${RENDER_HOSTNAME}`, { ...BASE_CONFIG, renderExternalHostname: `${RENDER_HOSTNAME} ` })).toBe(false);
  });

  it("does not widen ALLOWED_ORIGINS override behavior -- an explicit configured list still ignores the Render origin entirely", () => {
    const config = {
      ...BASE_CONFIG,
      configuredOrigins: ["https://staging.harveytaxiservice.com"],
      renderExternalHostname: RENDER_HOSTNAME
    };
    expect(isAllowedOrigin(`https://${RENDER_HOSTNAME}`, config)).toBe(false);
    expect(isAllowedOrigin("https://staging.harveytaxiservice.com", config)).toBe(true);
  });

  it("buildDefaultAllowedOrigins includes exactly one Render origin, never a pattern", () => {
    const set = buildDefaultAllowedOrigins({
      appBaseUrl: "https://harveytaxiservice.com",
      canonicalHost: "harveytaxiservice.com",
      foundationHost: "harveytransportationfoundation.com",
    ...
    expect(set.has(`https://${RENDER_HOSTNAME}`)).toBe(true);
    expect(set.has("https://some-other-app.onrender.com")).toBe(false);
    expect([...set].some((entry) => entry.includes("*"))).toBe(false);
  });

  it("buildDefaultAllowedOrigins tolerates a missing renderExternalHostname without throwing", () => {
    expect(() =>
      buildDefaultAllowedOrigins({ appBaseUrl: "https://x.com", renderExternalHostname: undefined })
    ).not.toThrow();
  });
});
