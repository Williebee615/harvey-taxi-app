...
// Six required proofs named explicitly per review, each as its own
// clearly-labeled test so the mapping from requirement to test is
// unambiguous on inspection — not relying on a reader to infer it from
// the more general composition tests above (which these deliberately
// overlap with in places; duplication here is for audit clarity, not
// because the underlying behavior is untested elsewhere).
describe("PR 3 — six required proofs (named per review checklist)", () => {
  ...
  // Express test client — see every prior PR's own doc in this series),
  // so an actual unauthenticated HTTP POST to
  ...
  // request — the same guard exercised for every migrated route.
  // Marked Unverified end-to-end in the PR doc; this is the closest
  // proof available short of live validation.
  test("1. unauthenticated identity resolves to no rider id, which the route's existing guard rejects", () => {
    ...
    expect(riderId).toBe("");
    // Mirrors every migrated route's own `if (!riderId) return fail(...)`.
  ...
  // verified instead by direct code-path tracing in PR 2c
  // (docs/security-remediation/pr-02c-signup-session-handoff.md):
  // rider-signup.html no longer offers card setup at all, pre- or
  // post-session; a rider only ever reaches payment-method actions from
  // the authenticated dashboard, which itself gates every rider-owned
  // call behind a confirmed GET /api/rider/session. No test asserted
  // here — recorded as a pointer so this requirement isn't silently
  // dropped, not faked with an assertion that doesn't actually test it.
  ...
  // property of this whole PR (a regression here would mean production
  // behavior changed the moment this PR merged, flag or no flag).
  test("6. with no authenticated identity (flag off / requireRiderIfEnforced passthrough), the client-supplied value is trusted exactly as before this PR", () => {
