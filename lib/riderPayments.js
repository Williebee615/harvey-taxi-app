...
// fare; the intent isn't already bound to a different ride; and, when
// this ride has an identified rider, the intent's rider metadata is
// present and matches (mandatory in that case, not merely checked when
// present — see the comment above that check). Returns { ok: true,
...
function verifyPaymentIntentForRide({ stripeConfigured, intent, ride, rideId, exposeDetails = false }) {
  ...
  // Required, not merely checked-when-present — a real Stripe PaymentIntent
  // always has a currency, so a missing one means this object didn't
  // actually come from Stripe (or is malformed in some other way that
  // makes it untrustworthy), and must fail the same as a mismatched one.
  if (intent.currency !== RIDE_PAYMENT_CURRENCY) {
    return {
      ok: false,
      error: "Payment currency does not match the ride fare.",
      statusCode: 402,
      extra: exposeDetails
        ? { expected_currency: RIDE_PAYMENT_CURRENCY, intent_currency: intent.currency ?? null }
        : {}
    };
  }
  ...
  // Mandatory whenever the ride itself is tied to an identified rider —
  // an intent with no rider_id metadata (or a mismatched one) must not be
  // allowed to authorize someone else's identified ride, which the old
  // "only check if intentRider is present" version allowed: an attacker
  // could create their own real, paid PaymentIntent with no rider_id at
  // all and use it to authorize any ride they could name, since an
  // unbound intent also passes the ride-binding check above. This is
  // intentionally *not* required when the ride has no rider_id at all —
  // POST /api/rides/request supports an anonymous ride request today (see
  // the `if (riderId)` optional readiness check there), and an anonymous
  // ride has no identified rider to bind the payment against in the first
  // place, so nothing changes for that pre-existing, unrelated path.
  const intentRider = intent.metadata?.rider_id;
  if (ride?.rider_id && intentRider !== String(ride.rider_id)) {
    return {
      ok: false,
...
// Wraps verifyPaymentIntentForRide with the one remaining side effect
// authorization needs: binding the intent to this ride via Stripe so it
// can't later be reused on a different one. bindPaymentIntentToRide is
// injected — the actual stripe.paymentIntents.update() call lives in
// server.js — so this orchestration, including what happens when the
// bind itself fails, is unit-testable without a live Stripe account, the
// same dependency-injected shape as sweepExpiredOffers()/
// sweepScheduledRides().
//
// A bind failure must not be treated as a warning and waved through: an
// intent that verified cleanly but couldn't be bound to this ride is not
// yet safe against reuse elsewhere, so authorization must stop here
// rather than proceeding to mark the ride PAYMENT_AUTHORIZED anyway.
async function authorizePaymentIntentForRide({
  stripeConfigured,
  intent,
  ride,
  rideId,
  ...
  const verification = verifyPaymentIntentForRide({ stripeConfigured, intent, ride, rideId, exposeDetails });

  if (!verification.ok || !verification.needsBinding) {
    return verification;
  }

  try {
    await bindPaymentIntentToRide();
  } catch (bindErr) {
    return {
      ok: false,
      error: "Could not secure this payment against reuse. Please try again.",
      statusCode: 502
    };
  }
