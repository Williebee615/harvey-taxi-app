...
// Follow-up fix (incident: a rate-limited reviewer login, HTTP 429, was
// indistinguishable client-side from a wrong password -- the dashboard
// only special-cased a 503 and collapsed every other non-2xx status into
// "Invalid reviewer credentials."). This is the exact client-facing
// message for a failed review-login attempt, given only the HTTP status
// and (when the server supplied one) retry_after_seconds -- pulled out
// as a pure function so this mapping is unit-tested once, here, rather
// than trusted to inline dashboard JavaScript to stay in sync with
// server.js's actual status codes.
//
// rider-dashboard.html's handleReviewSignIn and driver-dashboard.html's
// equivalent implement this identical logic directly in their own
// inline <script>, since a static HTML page can't require() a CommonJS
// module -- this function is the source of truth those two call sites
// are written to match, and are covered by this module's own tests.
// Never distinguishes "account not found" from "wrong password" (both
// collapse into the same generic "Invalid reviewer credentials." text)
// -- that's the same no-enumeration principle resolveReviewLoginOutcome
// already applies server-side, preserved here rather than reintroduced
// by a client that tried to be more specific.
function resolveReviewLoginClientMessage({ status, retryAfterSeconds }) {
  if (status === 503) {
    return "Google Play review sign-in is temporarily disabled.";
  }
  ...
  if (status === 429) {
    ...
    return "Too many sign-in attempts. Wait 10 minutes and try again.";
  }
