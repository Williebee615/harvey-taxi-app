...
// Finds the admin_roles row for a given email, matching case-
// insensitively. Phase 1's migration only guarantees a unique index on
// lower(email) -- it never guaranteed every stored `email` value is
// itself lowercase. A row written as "CaseWorker@HarveyTaxiService.com"
// must still resolve for a lookup of "caseworker@harveytaxiservice.com"
// (or any other casing), since the unique index already treats those
// as the same identity; comparing with exact-case equality against a
// row of unknown casing would silently miss real matches. server.js
// fetches all admin_roles rows (the table stays small -- one row per
// admin identity) and calls this pure function to find the match,
// rather than pushing case-folding into a database-level LIKE/ILIKE
// pattern that would need its own wildcard-escaping to stay an exact
// match.
function findAdminRoleRow(rows, email) {
  const target = typeof email === "string" ? email.trim().toLowerCase() : "";
  if (!target || !Array.isArray(rows)) return null;
  return (
    rows.find(
      (row) => row && typeof row.email === "string" && row.email.trim().toLowerCase() === target
    ) || null
  );
}
