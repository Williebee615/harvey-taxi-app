...
// The exact response contract for GET /api/rider/session (P0 remediation
// PR #2a) -- an explicit allow-list, not a projection of whatever fields
// happen to be convenient. Pulled out as its own pure function so this
// contract is enforced and tested here, once, rather than trusted to
// every future call site to remember: the riders row (req.rider, loaded
// by requireRider) carries payment fields (stripe_customer_id), auth
// artifacts, and other internal columns that this bootstrap endpoint --
// unauthenticated-until-this-call clients hitting it right after sign-in
// -- must never echo back. If a new sensitive column is ever added to
// riders, it is absent from this response by default, not by
// remembering to exclude it.
function buildRiderSessionBootstrap({ rider, readiness }) {
  return {
    rider_id: rider.id,
...
// Which riders-table column, if any, a successful login OTP should flip
// to true (P0 remediation PR 2a: "mark the appropriate phone/email
// verification field only if the verified OTP is valid for that
// channel" -- server.js's /api/rider/session/verify calls this only
// after Twilio Verify (phone) or verifyCode (email) has already
// confirmed the code, never before). Proving a phone code proves
// control of the phone, nothing about the email, and vice versa -- this
// never sets both from one successful channel. Returns null (no update
// needed) once the flag is already true, so a returning rider's every
// login doesn't re-write a column that hasn't changed.
function buildRiderVerificationFieldUpdate({ channel, rider }) {
  if (channel === "phone" && rider?.sms_verified !== true) {
    return { sms_verified: true };
  }

  if (channel === "email" && rider?.email_verified !== true) {
...
// The exact response contract for the public, unauthenticated
// GET /api/rider/auth-ui-config (P0 remediation PR 2a) -- a one-key
// allow-list, same reasoning as buildRiderSessionBootstrap above: this
// route's only job is "is the rider sign-in gate on," and it must never
// grow to expose the raw system_flags row, any other flag's key/value,
// environment details, or secret-presence indicators, even by accident
// in a later edit. Coerces to a strict boolean rather than passing
// through whatever getSystemFlag's string comparison produced, so a
// caller can't leak anything but true/false through this field either.
function buildAuthUiConfigResponse({ enabled }) {
  return { enabled: enabled === true };
}

// The IDOR fix itself, as a single pure decision (P0 remediation PR 2b,
// docs/security-remediation/pr-02b-rider-route-enforcement.md): once a
// route is migrated and rider_auth_enforced is on, an authenticated
// identity (from requireRiderIfEnforced -- see server.js) must always
// win over anything the client also sent, never merely be preferred or
// checked-against. There is deliberately no "fall back to
// clientSuppliedRiderId if it doesn't match" branch -- the whole point of
// this remediation is that a mismatched client value must be silently
// ignored, not compared and rejected (comparing and rejecting still
// requires trusting the client-shaped input to be meaningful at all).
// While the flag is off, requireRiderIfEnforced never sets
// authenticatedRiderId, so this always falls through to the pre-PR-2b,
// client-supplied behavior unchanged.
function resolveEnforcedRiderId({ authenticatedRiderId, clientSuppliedRiderId }) {
  if (authenticatedRiderId) {
    return authenticatedRiderId;
  }

  return clientSuppliedRiderId || "";
}
