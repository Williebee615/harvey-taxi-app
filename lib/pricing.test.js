const { calculateRideEstimate, hasValidTripDistance } = require("./pricing");
...
// Server-side enforcement of "no valid route + no verified fare = no
// payment and no dispatch." rider-dashboard.html already refuses to call
// the estimate/payment-intent/request routes until Google Maps has
// resolved a real distance, but that is only a client-side gate — a
// request sent directly to the API (bug, bypass, or malicious client)
// must still be rejected server-side instead of silently pricing the
// trip at the flat minimum fare via calculateRideEstimate()'s own
// miles=0/minutes=0 defaults.
describe("hasValidTripDistance", () => {
  it("accepts a real, positive resolved distance", () => {
    expect(hasValidTripDistance({ miles: 5.2, minutes: 14 })).toBe(true);
  });

  it("accepts zero minutes (a very short but real trip) as long as miles is positive", () => {
    expect(hasValidTripDistance({ miles: 0.3, minutes: 0 })).toBe(true);
  });

  it("rejects zero miles — the historical silent-minimum-fare bug", () => {
    expect(hasValidTripDistance({ miles: 0, minutes: 12 })).toBe(false);
  });

  it("rejects a missing miles field entirely", () => {
    expect(hasValidTripDistance({ minutes: 12 })).toBe(false);
    expect(hasValidTripDistance({})).toBe(false);
    expect(hasValidTripDistance()).toBe(false);
  });

  it("rejects negative miles or minutes", () => {
    expect(hasValidTripDistance({ miles: -1, minutes: 5 })).toBe(false);
    expect(hasValidTripDistance({ miles: 5, minutes: -1 })).toBe(false);
  });

  it("rejects non-numeric or non-finite values", () => {
    expect(hasValidTripDistance({ miles: "abc", minutes: 5 })).toBe(false);
    expect(hasValidTripDistance({ miles: NaN, minutes: 5 })).toBe(false);
    expect(hasValidTripDistance({ miles: Infinity, minutes: 5 })).toBe(false);
    expect(hasValidTripDistance({ miles: 5, minutes: undefined })).toBe(false);
  });

  it("accepts a numeric string miles value the same way calculateRideEstimate's Number() coercion would", () => {
    expect(hasValidTripDistance({ miles: "5.5", minutes: "10" })).toBe(true);
  });
});
